...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiFilm } from 'react-icons/fi';

import { useCinemaFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['team-member', 'business-owner', 'customer-service-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useCinemaFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Cinemas',
      path: isTenantUser ? `/cinemas/view/${data?.id}` : '/cinemas',
      entity: 'cinema',
