...
import { useRoqClient, useCinemaFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<CinemaInterface, unknown>;

interface CinemaListPageProps {
...
export function CinemaListPage(props: CinemaListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useCinemaFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.cinema.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: CinemaInterface) => {
    if (hasAccess('cinema', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/cinemas/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/cinemas/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('cinema', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/cinemas/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('cinema', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
